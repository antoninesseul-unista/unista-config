  ...
  <div>
    ...
    >
      <div v-if="activeParam" class="flex flex-col h-full">
        <div
          class="px-5 py-4 border-b flex justify-between items-center shrink-0"
          :class="
            activeParam.actif
...
import { equipmentStores, faultTypes } from "../core";
import {
  parseRobotMask,
  toggleRobotMask,
...
// Synchronous watch using pure TS parser
watch(
  () => props.activeParam?.robotMask,
  (mask) => {
    robotMask.value = parseRobotMask(mask);
  },
  { immediate: true },
);
...
// Synchronous toggle using pure TS bitwise operators (No IPC)
const toggleRobot = (robotIndex: number): void => {
  if (!props.activeParam) return;
  const newMask = toggleRobotMask(robotMask.value, robotIndex);
...
const setRobotVarIndex = (robotIndex: number, event: Event): void => {
  ...
  props.activeParam.robotVarIndex[robotIndex] = (raw === ""
    ? null
    : Number(raw)) as unknown as number;
};

/**
 * Ensures strict typing inside Array methods to fix "implicitly has an 'any' type" errors.
 */
const siblingParameters = computed((): models.Parameter[] => {
  ...
  for (const store of Object.values(equipmentStores)) {
    const owningEquipment = store.list.value.find((eq) =>
      (eq.parameters as models.Parameter[] | undefined)?.some(
        (p: models.Parameter) => p === props.activeParam,
      ),
    );
    if (owningEquipment) {
      return store.list.value.flatMap((eq) =>
        ((eq.parameters as models.Parameter[]) ?? []).filter(
          (p: models.Parameter) => p !== props.activeParam,
        ),
      );
    }
...
// Synchronous local validation checking
watch(
  ...
  () => {
    ...
    for (let robotIndex = 0; robotIndex < 32; robotIndex++) {
      ...
      // Pure TS validation (no IPC backend call)
      const result = validateRobotVarIndexForRobot(param, robotIndex, siblings);
...
const getRobotError = (robotIndex: number): string | null =>
  robotErrors.value[robotIndex] ?? null;
