<template>
  <div>
    <div v-if="activeParam" class="fixed inset-0 bg-gray-900/20 z-40" @click="$emit('close')" />
    ...
    >
      <div v-if="activeParam" class="flex flex-col h-full">
        <div
          ...
          :class="activeParam.actif ? 'bg-green-50 border-green-100' : 'bg-gray-50 border-gray-200'"
        >
        ...
        <div class="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          >
            <div class="flex items-center justify-between mb-2">
              ...
              >
              <AppIcon name="lock" :size="16" class="text-red-500" />
            </div>
            <div>
              <label class="text-[11px] font-bold text-red-500 uppercase tracking-wider mb-1 block"
            ...
            <div>
              <label class="block mb-1 text-[11px] font-bold tracking-wider text-red-500 uppercase"
              ...
              >
                <option v-for="fault in faultTypes" :key="fault.index" :value="fault.index">
          ...
          <div class="bg-white border border-gray-200 p-3.5 rounded-xl shadow-sm space-y-3">
            <div class="flex items-center justify-between">
              <span class="text-xs font-bold text-gray-700 uppercase tracking-wider">Visible (Active)</span>
              <label class="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" v-model="activeParam.actif" class="sr-only peer" />
                <div
            ...
            <div class="flex items-center justify-between" v-if="capabilities.showResetVisible">
              <span class="text-xs font-bold text-gray-700 uppercase tracking-wider">Reset Visible</span>
              <label class="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" v-model="activeParam.resetVisible" class="sr-only peer" />
                <div
          ...
          >
            <div class="flex items-center justify-between mb-3">
              <span class="text-xs font-bold text-teal-800 uppercase tracking-wider">Cycle Steps</span>
              <span class="text-xs font-bold px-2 py-0.5 rounded-full bg-teal-200 text-teal-800"
          ...
          >
            <h3 class="text-xs font-bold text-purple-800 uppercase tracking-wider">Robot Assignment</h3>
              ...
              >
                <label class="flex items-center justify-between cursor-pointer select-none">
                  <span
                    ...
                    :class="isRobotSelected(robot.index) ? 'text-purple-950 font-bold' : 'text-gray-600'"
                  >
                    R{{ robot.index }}
                    <span v-if="robot.name" class="text-xs font-normal text-gray-500 ml-1">({{ robot.name }})</span>
                  </span>
                  <input
                    type="checkbox"
                ...
                >
                  <div class="flex items-center justify-between gap-2">
                    <label class="text-[11px] font-bold text-purple-800 uppercase tracking-wider">
            ...
            <div v-if="capabilities.showRobotVarName" class="pt-1">
              <label class="block text-[11px] font-bold text-purple-700 uppercase tracking-wider mb-1.5">
                Variable Name
              </label>
              <input
                type="text"
          ...
          >
            <p class="text-xs text-gray-400 font-medium">No robots configured yet.</p>
          </div>

          <div v-if="capabilities.showTranslations && activeParam.commentFr !== undefined">
            <h3 class="text-xs font-bold text-gray-400 uppercase border-b pb-1.5 mb-2.5">Translations & Infos</h3>
            <div class="space-y-2.5">
              <div v-for="lang in LANGUAGE_FIELDS" :key="lang.code">
                ...
                >
                  <span :class="['text-[11px] px-1 py-0.5 rounded', lang.bg, lang.text]">{{ lang.code }}</span>
                  Comment
                </label>
                <input
                  type="text"
...
import { CalculationService, equipmentStores, faultTypes } from "../core";
const props = withDefaults(
  defineProps<{
    activeParam: models.Parameter | null;
...
const robotsList = computed(() => equipmentStores.robot.list.value.filter((r) => r.enable));

const robotMask = ref(0);
const robotErrors = ref<Record<number, string>>({});

watch(
  () => props.activeParam?.robotMask,
  async (mask) => {
    robotMask.value = await CalculationService.parseRobotMask(mask);
  },
  { immediate: true },
);

const isRobotSelected = (robotIndex: number): boolean => (robotMask.value & (1 << robotIndex)) !== 0;

const toggleRobot = async (robotIndex: number): Promise<void> => {
  if (!props.activeParam) return;
  const newMask = await CalculationService.toggleRobotMask(robotMask.value, robotIndex);
  robotMask.value = newMask;
...
const getRobotVarIndex = (robotIndex: number): number | string =>
  props.activeParam?.robotVarIndex?.[robotIndex] ?? "";

const setRobotVarIndex = (robotIndex: number, event: Event): void => {
  if (!props.activeParam) return;
  if (!props.activeParam.robotVarIndex || typeof props.activeParam.robotVarIndex !== "object") {
    props.activeParam.robotVarIndex = {};
  }
  const raw = (event.target as HTMLInputElement).value;
  props.activeParam.robotVarIndex[robotIndex] = (raw === "" ? null : Number(raw)) as unknown as number;
};

const siblingParameters = computed((): models.Parameter[] => {
  if (!props.activeParam) return [];

  for (const store of Object.values(equipmentStores)) {
    const owningEquipment = store.list.value.find((eq) => eq.parameters?.some((p) => p === props.activeParam));
    if (owningEquipment) {
      return store.list.value.flatMap((eq) => (eq.parameters ?? []).filter((p) => p !== props.activeParam));
    }
  }
  return [];
});
...
watch(
  [() => props.activeParam, siblingParameters, robotMask],
  async () => {
    ...
    const siblings = siblingParameters.value;

    for (let robotIndex = 0; robotIndex < 32; robotIndex++) {
      if (!isRobotSelected(robotIndex)) continue;
      const result = await CalculationService.validateRobotVarIndexForRobot(param, robotIndex, siblings);
      if (result.hasError) errors[robotIndex] = result.message;
    }

    robotErrors.value = errors;
  },
  { deep: true, immediate: true },
);

const getRobotError = (robotIndex: number): string | null => robotErrors.value[robotIndex] ?? null;
